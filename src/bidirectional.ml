...
(* let learn_rule_cache = ref BatMap.empty *)
...
let rec learn (available_height, available_size) (nt, desired_sig) 
	...
	if (BatMap.mem (nt, desired_sig, spec) !learn_cache) then
		BatMap.find (nt, desired_sig, spec) !learn_cache
	(* already consumed availble_size *)
	else if (available_height <= 0 || available_size <= 0) then Empty 
	else if (BatSet.mem (nt, desired_sig, spec) !now_learning) then
		let _ = prerr_endline (Printf.sprintf "*** Already learning *** %s %s" (string_of_rewrite nt) (AftaUtils.string_of_sig desired_sig)) in
			Empty
	else
	let _ = 
	...
	let result =
  	...
  	if (BatMap.mem (nt, desired_sig) nt_sig_to_expr) then
  		let _ = 
				my_prerr_endline (Printf.sprintf "Direct: (%s, %s) -> %s" 
					(Grammar.string_of_rewrite nt) 
					(string_of_sig desired_sig)
					(Exprs.string_of_expr (BatMap.find (nt, desired_sig) nt_sig_to_expr))
				) 
			in  
      	...
      	BatList.fold_left (fun acc (nt', rule) ->
          		...
          		if (vsa <> Empty) then
								let _ = 
									if (Pervasives.compare (nt, desired_sig) !goal) = 0 && not (!Options.find_all) then 
									(
										prerr_endline "solution found! - stop searching";
  					...
  					(* Rule: A -> B  *)
  						...
  						let vsa = 
    						learn (available_height, available_size) (nt', desired_sig) 
								  (spec, nt_rule_list, total_sigs, nt_to_sigs, nt_sig_to_expr)
    					in 
          		if (vsa <> Empty) then BatSet.add vsa acc else acc 
  					else acc 
  				else acc 
	...
	if (lb <= available_size) then
		(* caching *)
  	let _ = learn_cache := BatMap.add (nt, desired_sig, spec) result !learn_cache in
...
and learn_rule (available_height, available_size) (nt, desired_sig) 
	...
	let result =  
  	...
  	begin
    		...
    		(* single permutation: process args in order starting from the first one *)
    		(* obtain all [(nt1, sig1), ..., (ntn, sign)] *)
    		let nt_sig_lists =
					learn_paths 0 rule nts desired_sig total_sigs nt_to_sigs BatSet.empty
				in
    		if (BatSet.is_empty nt_sig_lists) then Empty 
    		else 
    		(* Learn each, and union them *)
    			let vsas =
						try 
    				BatSet.fold (fun nt_sig_list acc ->
        					...
        					BatList.fold_left (fun (available_size', acc) (nt, sg) ->
      							let vsa = 
  										learn (available_height - 1, available_size') (nt, sg) 
											  (spec, nt_rule_list, total_sigs, nt_to_sigs, nt_sig_to_expr) 
  									in
  									(* if an argument is empty, no need to proceed any more *)
        				...
        				in 
							...
							(* if an argument is empty, try next combination *)
	...
	in
	(* caching *) 
  (* let _ = learn_rule_cache := BatMap.add (nt, desired_sig, rule) result !learn_rule_cache in  *)
  result
...
and learn_for_each pts (available_height, available_size) (nt, desired_sig) 
	...
	let f (available_height, available_size) i =
		...
		let nt_rule_list = 
			List.filter (fun (nt, rule) -> 
				try (String.compare (Grammar.op_of_rule rule) "ite") <> 0 
				with _ -> false
			) nt_rule_list 
		in 
		let nt_sig_to_expr = 
	...
	in
	(* let vsa_list = List.map f (BatList.range 0 `To ((List.length desired_sig) - 1)) in *)
	vsa_list
		
(* run scancoarsen to get minimal covers( : set of sigs) of desired_sig. *)
(* 		each cover also should be of nt. *)
(* 		exists i. cover[i] = desired_sig *)
(* 		forall i. exists cover in covers. desired_sig[i] = cover[i] *)
...
and learn_ite (available_height, available_size) (nt, desired_sig) 
	...
	let _ = 	
		if (Pervasives.compare (snd !goal) desired_sig) = 0 then
			prerr_endline (Printf.sprintf "learn_ite")
	in
	(* for each input, learn vsa *)
	let pts_to_cover =
	...
	let (nt_to_sigs, nt_sig_to_expr) =
    	...
    	let _ = 	
    		if (Pervasives.compare (snd !goal) desired_sig) = 0 then
    			prerr_endline ("learn for each start")
    	in
			let vsa_list =
    	...
    	in
			let _ = 	
    		if (Pervasives.compare (snd !goal) desired_sig) = 0 then
    			prerr_endline ("learn for each ends")
    	in
  		(* add learned exprs into sigs and exprs *)
  		List.fold_left (fun (nt_to_sigs, nt_sig_to_expr) vsa ->
  			...
  			with NoSolInVSA -> assert false  (* exception should have been raised earlier *)
				| UndefinedSemantics -> (nt_to_sigs, nt_sig_to_expr)
  		) (nt_to_sigs, nt_sig_to_expr) vsa_list
	in
	(* should be here *)
	let nt_sigs = BatMap.find nt nt_to_sigs in
	let _ = if (not_covered nt_sigs desired_sig) <> [] then raise NoSolInVSA in   
	(* let nt_sigs = scan_coarsen (BatSet.empty, nt_sigs) desired_sig in *)
	...
	let rec learndt pts bool_sigs =
		...
		if not (BatSet.is_empty nt_sigs) then
			...
			(* DEPRECATED: learn predicates and add them to the predicate pool *)
			(* let bool_sig =                                                              *)
			(* 	List.fold_left (fun acc i ->                                              *)
			(* 		if (List.mem i pts) then acc @ (CBool true) else acc @ (CBool false)    *)
			...
			in
			(* learn additional predicates if no pred remains*)
			let bool_sigs, nt_sig_to_expr =
				if (BatSet.is_empty bool_sigs) then
							...
							if (acc <> None) || pts_index1 = pts_index2 then acc
  							...
  							let bool_sig =
    							List.fold_left (fun bool_sig i ->
  									let const =
  										if (i = pts_index1) then (CBool true)
  										else if (i = pts_index2) then (CBool false)
  										else (CInt 0)
  									in
    								bool_sig @ [const]
    							) [] (BatList.range 0 `To ((List.length desired_sig) - 1))
					...
					if pred_opt = None then (bool_sigs, nt_sig_to_expr)
						...
						let bool_sig = try Exprs.compute_signature spec pred with _ -> assert false in
  		...
  		(* 		prerr_endline (Printf.sprintf "pred: %s" (Exprs.string_of_expr pred));          *)
			...
			(* 	end                                                                               *)
  		(* in                                                                                  *)
			let left = learndt left_pts bool_sigs' in
			let right = learndt right_pts bool_sigs' in
	...
	let expr =
		...
		if !Options.lbu then 
  		...
  		learndt (BatList.range 0 `To ((List.length desired_sig) - 1)) bool_sigs
		(* for BV *)
		else
    ...
    let cover : (signature, int BatSet.t) BatMap.t =
    	BatSet.fold (fun nt_sig cover ->
    		...
    		in
    		(* prerr_endline (Printf.sprintf "covered: %s" (string_of_set string_of_int covered_pts)); *)
    ...
    in
		...
		=
			...
			try
				...
				let nt_to_sigs = BatMap.add nt (BatSet.filter (fun sg -> (BitSet.cardinal (BatMap.find sg cover)) > 0) nt_sigs) nt_to_sigs in
				prerr_endline (Printf.sprintf "pred_size: %d |bool_sigs|: %d |nt_to_sigs|:%d" pred_size (BatSet.cardinal bool_sigs) (BatMap.cardinal nt_to_sigs));
    		(* learndt pts_indices bool_sigs *)
    		Dtlearning.learndt
  		...
  		with LearnDTFailure ->
				...
				let nt_to_exprs : (Grammar.rewrite, (Exprs.expr * int) BatSet.t) BatMap.t  =
					...
					BatMap.foldi (fun nt sigs nt_to_exprs ->
						let exprs = set_map (fun sg -> let expr = BatMap.find sg sig_to_expr in (expr, size_of_expr expr)) sigs in
						BatMap.add nt exprs nt_to_exprs
					) nt_to_sigs BatMap.empty
				in
  			let (nt_to_sigs, nt_to_exprs, nt_sig_to_expr) =
    			let nt_rule_list =
    				List.filter (fun (nt, _) -> (Pervasives.compare nt bool_nt) = 0) nt_rule_list
    			in
      	...
      	in
				let bool_sigs = BatSet.filter (fun sg -> (type_of_signature sg) = Bool) total_sigs in
	...
	let _ = 
		if (Pervasives.compare (snd !goal) desired_sig) = 0 then
		begin
			prerr_endline ("ite sol: " ^ (Exprs.string_of_expr expr));
			prerr_endline ("available size: " ^ (string_of_int available_size));
			prerr_endline ("size: " ^ (string_of_int (Exprs.size_of_expr expr)))
		end 
	in
	Direct expr
	with LearnDTFailure ->
		let _ = if (Pervasives.compare desired_sig (snd !goal)) = 0 then 
  		prerr_endline ("learn dt failure") 
  	in 
		Empty
	| LearnForEachFailure ->
		let _ = if (Pervasives.compare desired_sig (snd !goal)) = 0 then 
  		prerr_endline ("learn for each failure") 
  	in 
		Empty
	| NoSolInVSA ->
		let _ = if (Pervasives.compare desired_sig (snd !goal)) = 0 then 
  		prerr_endline ("no sol in vsa") 
  	in 
		Empty
...
let synthesis (macro_instantiator, target_function_name, grammar, forall_var_map, spec) =
	...
	in    
	prerr_endline ("max height: " ^ (string_of_int max_height));
	...
	let rec iter max_component_size (nt_to_sigs, nt_to_exprs, nt_sig_to_expr) =
		...
		let _ = if max_component_size > !Options.max_size then failwith ("No solution within size of " ^ (string_of_int !Options.max_size)) in 
  	let prior_size_nt_sig_to_expr = BatMap.cardinal nt_sig_to_expr in 
		let (nt_to_sigs, nt_to_exprs, nt_sig_to_expr) =
			(* exclude ite's*)
			(* when we add components, we don't have to add components of ite; *)
			(* they will be computed by our divide-and-conquer algorithm *)
			let nt_rule_list = 
				List.filter (fun (nt, rule) -> not (BatString.equal "ite" (try op_of_rule rule with _ -> ""))) nt_rule_list 
			in
  		get_sigs_of_size desired_sig spec (nt_to_sigs, nt_to_exprs, nt_sig_to_expr) 
				nt_rule_list (max_component_size, max_component_size + 1) 
  	in 
		prerr_endline (Printf.sprintf "max_component_size : %d - #components: %d" max_component_size (BatMap.cardinal nt_sig_to_expr));
		if (BatMap.cardinal nt_sig_to_expr) = prior_size_nt_sig_to_expr || max_component_size < !Options.init_comp_size then
		...
		try
  		...
  		(* 	prerr_endline (Printf.sprintf "%s -> %s" (Grammar.string_of_rewrite nt) (string_of_set AftaUtils.string_of_sig sigs))                                             *)
  		(* ) nt_to_sigs;                                                                                                                                                       *)
  		let sol =
      	let total_sigs =
      		BatMap.foldi (fun nt sigs total_sigs ->
      	...
      	in
				(* prerr_endline ("************ nt, sig -> expr ************"); *)
    		(* BatMap.iter (fun (nt, sg) expr ->                            *)
    		(* 	prerr_endline (                                            *)
				(* 		Printf.sprintf "%s, %s -> %s"                            *)
				(* 			(Grammar.string_of_rewrite nt)                         *)
				(* 			(AftaUtils.string_of_sig sg)                           *)
				(* 			(Exprs.string_of_expr expr)                            *)
				(* 	)                                                          *)
    		(* ) nt_sig_to_expr;                                            *)
      	...
      	my_prerr_endline ("Components are ready to be used: ");
				...
				let nt_rule_list =
					if (List.length desired_sig) = 1 then  
  					List.filter (fun (nt, rule) -> not (BatString.equal "ite" (try op_of_rule rule with _ -> ""))) nt_rule_list
					else nt_rule_list 
  			in
				let vsa =
				...
				in
      	prerr_endline ("VSA has been computed");
	...
	(* a solution is found while preparing components *) 
		sol  
